...
This repository implements a **Functional Causal Inference** pipeline using **Functional Propensity Score (FPS) weighting** and **Marginal Structural Models (MSMs)** to evaluate the causal impact of midlife Body Mass Index (BMI) trajectories on late-life health outcomes. Using longitudinal data from the **RAND Health and Retirement Study (HRS 1992–2022)**, this project investigates: (1) **Study 1 (Scalar MSM)** on the effect of BMI trajectories ($t \in [52, 64]$) on the risk of late-onset incident Type 2 Diabetes ($t \in (64, 76]$), and (2) **Study 2 (Function-on-Function MSM)** on the causal effect surface relating midlife BMI trajectories ($s \in [52, 62]$) to subsequent cognitive performance trajectories (`cog27`, $t \in [64, 74]$).
